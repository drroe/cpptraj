#ifndef INC_EWALD_PARTICLEMESH_PERATOM_H
#define INC_EWALD_PARTICLEMESH_PERATOM_H
#ifdef LIBPME
#include <vector>
...
class Box;
class Topology;
/// Class implementing a per-atom decomposed version of PME
class Ewald_ParticleMesh_PerAtom : private Ewald_ParticleMesh {
  public:
    Ewald_ParticleMesh_PerAtom();
    ...
    /// Calculate per-atom nonbonded energy with PME
    int CalcNonbondEnergy_PerAtom(Frame const&, AtomMask const&,
                                  double&, double&);
...
*/
  private:
    ...
    double Self_PerAtom(double, Darray&);
    /// Lennard-Jones self energy, decomposed onto atoms.
    double Self6_PerAtom(Darray&);
    /// Reciprocal energy decomposed for every atom.
    double Recip_ParticleMesh_PerAtom(Box const&, MatType&);
    /// LJ reciprocal term, decomposed for every atom.
    double LJ_Recip_ParticleMesh_PerAtom(Box const&, MatType&);
    /// VDW long range correction, per atom
    double Vdw_Correction_PerAtom(double, Darray&);
    /// Calculate direct space energy, decomposed for every atom.
    double Direct_PerAtom(PairList const&, double&);
    /// Calcualte direct space energy with long range VDW correction, decomposed for every atom.
    double Direct_VDW_LongRangeCorrection_PerAtom(PairList const&, double&);
    /// Calculate direct space energy with LJ PME, decomposed for every atom.
    double Direct_VDW_LJPME_PerAtom(PairList const&, double&);

...
Name GenTempName() {
  ...
  Name temp( TempPrefix_ + integerToString(tmpidx) );
  while (tmpidx < maxtmpidx_ && File::Exists(temp)) {
    tmpidx++;
    temp = Name( TempPrefix_ + integerToString(tmpidx) );
  }
...
void FreeTempName(Name const& temp) {

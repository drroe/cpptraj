#ifdef LIBPME
#include <algorithm>
#include "Ewald_ParticleMesh_PerAtom.h"
#include "Frame.h"
...
/** CONSTRUCTOR */
Ewald_ParticleMesh_PerAtom::Ewald_ParticleMesh_PerAtom() {}

/** Allocate internal arrays. */
int Ewald_ParticleMesh_PerAtom::AllocateArrays(unsigned int natoms, unsigned int nthreads)
{
...
/** Calculate full nonbonded energy with PME, decomposed for every atom.
 */
int Ewald_ParticleMesh_PerAtom::CalcNonbondEnergy_PerAtom(Frame const& frameIn, AtomMask const& maskIn,
                                      double& e_elec, double& e_vdw)
{
  ...
  double e_self = Self_PerAtom( volume, E_elec_self_ );

  // Create pairlist
  int retVal = pairList_.CreatePairList(frameIn, frameIn.BoxCrd().UnitCell(), frameIn.BoxCrd().FracCell(), maskIn);
  if (retVal != 0) {
    mprinterr("Error: Ewald_ParticleMesh_PerAtom: Grid setup failed.\n");
    return 1;
  }
  ...
  double e_recip = Recip_ParticleMesh_PerAtom( frameIn.BoxCrd(), e_potentialD_ );
  ...
  if (lw_coeff_ > 0.0) {
    ...
    e_vdw6self = Self6_PerAtom(E_vdw_self_);

    MatType vdw_potentialD(natoms, 4);
    vdw_potentialD.setConstant(0.0);
    e_vdw6recip = LJ_Recip_ParticleMesh_PerAtom( frameIn.BoxCrd(), vdw_potentialD );
    ...
    e_vdw_lr_correction = Vdw_Correction_PerAtom( volume, E_vdw_lr_cor_ );
    //mprintf("e_vdw_lr_correction: %f \n", e_vdw_lr_correction);
  }

  e_vdw = 0.0;
  double e_direct = Direct_PerAtom( pairList_, e_vdw );
...
double Ewald_ParticleMesh_PerAtom::Self_PerAtom(double volume, Darray& atom_self) {
  t_self_.Start();
...
double Ewald_ParticleMesh_PerAtom::Self6_PerAtom(Darray& atom_vdw_self) {
...
/** PME recip calc to store decomposed reciprocal energy for every atom. */
double Ewald_ParticleMesh_PerAtom::Recip_ParticleMesh_PerAtom(Box const& boxIn, MatType& potential)
{
  t_recip_.Start();
...
double Ewald_ParticleMesh_PerAtom::LJ_Recip_ParticleMesh_PerAtom(Box const& boxIn, MatType& potential)
{
  t_recip_.Start();
  int nfft1 = Nfft(0);
  int nfft2 = Nfft(1);
...
double Ewald_ParticleMesh_PerAtom::Vdw_Correction_PerAtom(double volume, Darray& e_vdw_lr_cor) {
...
/** Direct space routine . */
double Ewald_ParticleMesh_PerAtom::Direct_PerAtom(PairList const& PL, double& evdw_out)
{
  if (lw_coeff_ > 0.0)
    return Direct_VDW_LJPME_PerAtom(PL, evdw_out);
  else
    return Direct_VDW_LongRangeCorrection_PerAtom(PL, evdw_out);
}

/** Nonbond energy kernel. */
void Ewald_ParticleMesh_PerAtom::Ekernel_NB(double& Eelec, double& Evdw,
                          double rij2,
                          double q0, double q1,
                          int idx0, int idx1,
...
void Ewald_ParticleMesh_PerAtom::Ekernel_Adjust(double& e_adjust,
                              double rij2,
                              double q0, double q1,
                              int idx0, int idx1,
...
/** Direct space calculation with long range VDW correction. */
double Ewald_ParticleMesh_PerAtom::Direct_VDW_LongRangeCorrection_PerAtom(PairList const& PL, double& evdw_out)
{
  ...
  //mprintf("Entering Direct_VDW_LongrangeCorrection_PerAtom function \n");
# ifndef _OPENMP
...
/** Direct space calculation with LJ PME. */ // TODO enable
double Ewald_ParticleMesh_PerAtom::Direct_VDW_LJPME_PerAtom(PairList const& PL, double& evdw_out)
{
  mprinterr("Error: LJPME does not yet work with PerAtom.\n");
  return 0;
/*
  t_direct_.Start();
  double Eelec = 0.0;

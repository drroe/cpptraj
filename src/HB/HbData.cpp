...
std::vector<int> HbData::GetRankNhbonds( int num_hb, Parallel::Comm const& commIn )
{
...
void HbData::HbondToArray(std::vector<double>& Dvals, std::vector<int>& Ivals, Hbond const& hb)
{
...
int HbData::SyncToMaster() {
  // Sanity check
  if (trajComm_.IsNull()) {
    mprinterr("Internal Error: HbData::SyncToMaster(): Across-trajectory comm is null.\n");
    return 1;
  }
  ...
  std::vector<unsigned int> rank_frames( trajComm_.Size() );
  trajComm_.GatherMaster( &Nframes_, 1, MPI_UNSIGNED, &rank_frames[0] );
  if (trajComm_.Master()) {

...
/** Solves a cubic equation: ax^3 + bx^2 + cx + d = 0 using "Cardan's formula" 
  ...
  */
double LeastSquaresPlane::solve_cubic_eq(double a, double b, double c, double d) {
...
/** Calcs (least-squares best) plane through a series of points
  ...
  */
Vec3 LeastSquaresPlane::leastSquaresPlane(unsigned int n, const double* vcorr) {
...
void LeastSquaresPlane::CalcLeastSquaresPlane(Frame const& currentFrame, AtomMask const& maskIn)
{
  ...
  nxyz_ = leastSquaresPlane(vcorr_.size(), &vcorr_[0]);
}

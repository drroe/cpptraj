...
/** Attempt to assign a reasonable values for phi internal coordinates for
  * atoms around a central bond j and k.
  */
int Cpptraj::Structure::Model::AssignPhiAroundBond(std::vector<InternalCoords>& IC,
                                                   ...
                                                   int debug)
{
  // Figure out hybridization and chirality of atom j.
  //if (debug > 0)
    mprintf("DEBUG: AssignPhi for bond %s - %s\n", topIn.AtomMaskName(ax).c_str(), topIn.AtomMaskName(ay).c_str());

  // Both need to have more than 1 bond
  if (topIn[ax].Nbonds() < 2 || topIn[ay].Nbonds() < 2) {
    mprintf("DEBUG: Not possible to make a torsion (%i bonds, %i bonds)\n", topIn[ax].Nbonds(), topIn[ay].Nbonds());
    return 0;
  }
  // Get hybridization of atoms x and y
  ParmHolder<AtomType>::const_iterator atx = AT.GetParam( TypeNameHolder(topIn[ax].Type()) );
  ParmHolder<AtomType>::const_iterator aty = AT.GetParam( TypeNameHolder(topIn[ay].Type()) );
  AtomType::HybridizationType hx, hy;
  if (atx != AT.end() && atx->second.Hybridization() == AtomType::UNKNOWN_HYBRIDIZATION)
    hx = GuessAtomHybridization( topIn[ax], topIn.Atoms() );
  else
    hx = atx->second.Hybridization();
  if (aty != AT.end() && aty->second.Hybridization() == AtomType::UNKNOWN_HYBRIDIZATION)
    hy = GuessAtomHybridization( topIn[ax], topIn.Atoms() );
  else
    hy = aty->second.Hybridization();

  int aj, ak;
  // Ensure hybridization of atom X is > atom Y
  if (hx < hy) {
    aj = ay;
    ak = ax;
  } else {
  ...
  mprintf("DEBUG: In hybridization order: %s - %s (%i - %i)\n", topIn.AtomMaskName(aj).c_str(), topIn.AtomMaskName(ak).c_str(),
          (int)hx, (int)hy);

  Atom const& AJ = topIn[aj];
  Atom const& AK = topIn[ak];

/*
  BuildAtom AtomJ;
  if (AJ.Nbonds() > 2) {
    Cpptraj::Structure::SetPriority(AtomJ.ModifyPriority(), aj, topIn, frameIn, 0); // FIXME chiralityDebug
  }
